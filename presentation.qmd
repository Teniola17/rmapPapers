---
title: "Progress after Upgrading"
format: beamer
editor: visual
---

Reviewed the codes (STAN code and the R codes) for possible bugs.

## Replicated the Viele et al. (2013)

This was suggested by an assessor on the upgrading day to validate the methodology proposed during the upgrading.
...
#---------------------------
library(ggplot2)
ggplot(results, aes(p0, type1, color = method)) +
  geom_line(size = 1.2) +
  geom_hline(yintercept = alpha, linetype = 2) +
  labs(title = "Type I error across control rates", y = "Type I Error", x = "True control rate")
...
ggplot(results, aes(p0, power, color = method)) +
  geom_line(size = 1.2) +
  labs(title = "Power across control rates", y = "Power", x = "True control rate")
...
ggplot(results, aes(p0, mse, color = method)) +
  geom_line(size = 1.2) +
  labs(title = "Mean Square Error of Control Estimate", y = "MSE", x = "True control rate")


```

## Explored the MAP and RMAP methodologies.

The possibilities of borrowing on both the intercept and the prognostic scores using the MAP and RMAP.

## Options for future works

Some further questions of interest are:\

-   Do borrowing depend on covariates as well as the outcomes?

-   Can the accuracy of different PS models (e.g. hierarchical for multiple historical studies) influence the Operating Characteristic properties of the model?

-   Is model mis-specification an issue for small trial?
